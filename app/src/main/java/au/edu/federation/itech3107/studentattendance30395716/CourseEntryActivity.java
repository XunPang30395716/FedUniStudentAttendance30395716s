...
public class CourseEntryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final EditText inputCourseId = findViewById(R.id.edtCourseId);
        final  EditText inputCourseName = findViewById(R.id.edtCourseName);
        displayStartDate = findViewById(R.id.tvStartDate);
        ...
        storeButton.setOnClickListener(v -> {
            ...
            // Check if the date has a single numeric month or day
            String[] parts = inputDate.split("-");
            if (parts.length == 3) {
                if (parts[1].length() == 1) {
                    parts[1] = "0" + parts[1];  // Fill 0 to month
                }
                if (parts[2].length() == 1) {
                    parts[2] = "0" + parts[2];  // Fill 0 to date
                }
